...
int fs_read(int fd, void *buf, size_t nbyte)
{
  ...
  while (bytes_left){
    ...
    memcpy((char *)buf + bytes_read, read_buffer + byte_offset, current_read);
...
int fs_write(int fd, const void *buf, size_t nbyte)
{
  ...
  */
  ...
  for (int i = 0; i < block_to_write; i++){
    ...
    memcpy(write_buffer + byte_offset,(char *)buf + bytes_written, current_write);
...
int fs_truncate(int fd, off_t length){
  ...
  struct inode * node = &inode_list[open_fd_list[fd].inode_num];
